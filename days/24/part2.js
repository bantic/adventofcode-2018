...
function createPart1Armies(boost) {
  let armies = {};
  armies.immune = [
  ...
  armies.immune.forEach(g => (g.attackHp = g.attackHp + boost));
  return armies;
}
...
module.exports = async function main(lines) {
  let boost = 43;
  while (true) {
    if (doesImmuneWin(boost)) {
      console.log('worked for boost:', boost, doesImmuneWin(boost));
      return;
    } else {
      console.log('no for boost', boost);
    }
    boost += 1;
  }
};

function doesImmuneWin(boost) {
  let armies = createPart1Armies(boost);
  while (armies.immune.length > 0 && armies.infection.length > 0) {
    // console.log(
    //   armies.immune.map(g => g.units).join(','),
    //   '----',
    //   armies.infection.map(g => g.units).join(',')
    // );
  ...
  if (armies.immune.length > 0) {
    return armies.immune;
  } else {
    return false;
  }
}
