...
module.exports = async function(lines) {
  ...
  let maxSleepMinute = null;
  let maxSleepsOnMinute = 0;
  Object.keys(sleep).forEach(guardId => {
    let minutes = sleep[guardId].minutes;
    Object.keys(minutes).forEach(minute => {
      let occurrences = minutes[minute];
      if (occurrences > maxSleepsOnMinute) {
        maxSleepGuard = guardId;
        maxSleepMinute = minute;
        maxSleepsOnMinute = occurrences;
      }
    });
  });
  console.log(maxSleepGuard, maxSleepMinute, maxSleepsOnMinute);
};
